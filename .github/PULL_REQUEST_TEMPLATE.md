...
- [ ] Any new method is in `exerciseEverything()` **and** the `INVENTORY` constant in `tests/ConformanceTest.php`
...
- [ ] Amounts stay integers in minor units, and every float is still rejected
- [ ] No transport failure can escape the error taxonomy, including on the body read
- [ ] No voucher code can reach a log, a `var_dump()`, or an exception message
