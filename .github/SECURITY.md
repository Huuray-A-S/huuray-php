...
- The library redacts voucher codes, credentials and contact details from anything it prints, exceptions keep only a redacted copy of a response body, and `var_dump()` / `print_r()` of a voucher show its code, CVV and redeem link as `[redacted: bearer value]`. If you add a field that carries value or personal data, add it to `SECRET_FIELDS` or `SENSITIVE_FIELDS` in [`src/Redact.php`](../src/Redact.php) and to the `__debugInfo()` of the class that carries it, and cover it with a test.
- Never log a request body from the order endpoints without passing it through `Redact::redact()` first.
